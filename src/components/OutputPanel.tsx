...
export default function OutputPanel({ output, error, isRunning }: OutputPanelProps) {
  return (
    <div className="h-full flex flex-col bg-gray-900 dark:bg-gray-950 transition-colors">
      <div className="flex items-center justify-between px-4 py-2 bg-gray-800 dark:bg-gray-900
        ...
        <span className="text-sm font-medium text-gray-300 dark:text-gray-400">输出</span>
        {isRunning && (
      ...
      <div className="flex-1 overflow-auto p-4 font-mono text-sm">
        {!output && !error && !isRunning && (
          <div className="text-gray-500 dark:text-gray-600 italic">点击「运行」按钮执行代码</div>
        )}
        {output && (
          <pre className="text-green-400 dark:text-green-300 whitespace-pre-wrap mb-2">{output}</pre>
        )}
        {error && (
